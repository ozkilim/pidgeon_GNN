...
def get_potential(sim, sim_obj):
    ...
    #   test_dist(jax.device_get(np.sum(np.any(dist <= min_dist))))
      ...
      elif sim == 'lj': # try with lenourd jones potential
          epsilon = 10  # energy in units of k_BT
          sigma = 1 # distance in units of sigma
          potential = 4.0 * epsilon * ((sigma / bounded_dist)**12 - (sigma / bounded_dist)**6) 
          return potential
...
class SimulationDataset(object):
    ...
    def simulate(self, ns, key=0):
        ...
        params = 2 #what are these perams?
        total_dim = dim*2+params #this is key to understanding the structure of the sim. understand what each part of the vec is... 
        times = self.times 
        G = self.G
        ...
        def make_sim(key):
            if sim in ['string', 'string_ball']:
                ...
                # last two parts are mass and charge?... what are the first 4?... x,y,dx,dy?
            ...
            # print(x0) $contains the 6 dims... 
            # print(x0.shape) (4,6)
            # print(packed_shape) #24
            # print(x_times) (500,4,6)
            return x_times
    ...
    def get_acceleration(self):
        ...
        dim = self._dim 
        sim = self._sim
        params = 2 # descriptors of the each particle like mass/chgarge... we could use hierarchy?...
        total_dim = dim*2+params # becuase there is position and velicty 
        times = self.times
        G = self.G
        if self.extra_potential is not None:
          vex = vmap(self.extra_potential, 0, 0)
